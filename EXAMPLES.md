...
**Scenario:** review feedback comes back with three items, one of them
worded vaguely ("needs proper validation").
**You say:** nothing — this triggers as soon as feedback arrives, on
either side of a review.
**What happens:** reads all three items first, asks one specific
question about the vague one instead of guessing at scope, then checks
the other two against the actual code — one turns out to conflict with a
comment explaining why the current behavior is intentional, so it's
pushed back on with the specific line that shows why, instead of
implemented on agreement alone.
