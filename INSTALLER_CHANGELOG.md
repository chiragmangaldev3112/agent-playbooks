...
fetched release: a relpath key from the archive JSON (an absolute path, or
one containing a `..` segment) was written to disk with no validation,
so a compromised or misconfigured backend could write outside the
temporary staging directory to anywhere the installing user has
permission — confirmed with a crafted key before this fix, not just by
inspection. Not exploitable today given the backend's locked-down RLS
setup (see `supabase/schema.sql`), but the installer itself shouldn't
depend entirely on that never slipping. Also synced `supabase/schema.sql`
and added `supabase/functions/check-in/index.ts` to this repo — both had
drifted out of date/missing here, which meant the README's "read it
yourself" / "run your own backend against the same schema" claims didn't
actually hold up against what was really shipped.
